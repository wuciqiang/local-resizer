    ...
    <div className="animate-fade-up">
      <div className="bg-gradient-to-r from-teal-600 to-emerald-500 rounded-2xl p-5 text-white mb-4 shadow-soft-lg">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-teal-100 text-sm mb-0.5">{totalChangeLabel}</p>
            <p className="text-2xl font-[var(--font-heading)] font-bold">
              {fmtBytes(totalChange.bytes)}
            </p>
          </div>
          <div className="text-right">
            <p className="text-teal-100 text-sm mb-0.5">{results.length} file{results.length > 1 ? 's' : ''}</p>
            <p className="text-2xl font-[var(--font-heading)] font-bold">
              {totalChange.direction === 'same' ? '' : totalChangePrefix}{totalChange.percent}%
            </p>
          </div>
        </div>
      </div>
...
function ResultListItem({
  ...
  const itemChange = sizeChange(result.originalSize, result.processedSize);
  const itemChangePrefix = itemChange.direction === 'increased' ? '+' : '-';
  const itemChangeClass = itemChange.direction === 'increased'
    ? 'ml-2 text-amber-600 font-medium'
    : 'ml-2 text-emerald-600 font-medium';
  const formatLabel = outputFormatLabel(result.outputFormat);

  return (
    <div role="listitem" className="bg-white rounded-xl border border-stone-200 shadow-soft p-3 sm:p-4">
        ...
        <div className="flex items-start gap-3 sm:gap-4 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-stone-600 mt-0.5">
              ...
              <span className={itemChangeClass}>
                {itemChange.direction === 'same' ? '' : itemChangePrefix}{itemChange.percent}%
              </span>
            </p>
            <p className="text-xs text-stone-500 mt-1.5">
              ...
              {formatLabel && (
                <span className="ml-2 text-blue-600 font-medium">{formatLabel}</span>
              )}
